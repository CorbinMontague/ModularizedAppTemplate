...
public class Module1ViewController: UIViewController {
    ...
    private let moduleAvatar: UIImageView = {
        let image = UIImage(named: "moduleAvatar", in: Bundle(for: Module1ViewController.self), compatibleWith: nil)
        let imageView = UIImageView(image: image)
        imageView.translatesAutoresizingMaskIntoConstraints = false
        return imageView
    }()
    ...
    private lazy var titleLabel: UILabel = {
        let label = UILabel()
        label.text = String(format: localize.string(forKey: "greetingModule", comment: "Welcome to BRAND"), brand.displayName, "Module1")
